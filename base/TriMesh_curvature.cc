                          ...
                          vec &new_u, vec &new_v)
{
	new_u = old_u;
	new_v = old_v;
	vec old_norm = old_u TRICROSS old_v;
...
{
	vec r_new_u, r_new_v;
	rot_coord_sys(new_u, new_v, old_u TRICROSS old_v, r_new_u, r_new_v);
...
//#pragma omp parallel for
	for (int i = 0; i < nv; i++) {
		...
		normalize(pdir1[i]);
		pdir2[i] = normals[i] TRICROSS pdir1[i];
	}
